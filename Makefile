...
# postworldd is a separate module under ./postworldd
EVMD_DIR      := postworldd
EVMD_MAIN_PKG := ./cmd/postworldd

###############################################################################
###                        Build & Install postworldd                             ###
###############################################################################
...
build: go.sum $(BUILDDIR)/
	@echo "🏗️  Building postworldd to $(BUILDDIR)/$(EXAMPLE_BINARY) ..."
...
# Install into $(BINDIR)
install: go.sum
	@echo "🚚  Installing postworldd to $(BINDIR) ..."
	@cd $(EVMD_DIR) && CGO_ENABLED="1" \
...
PACKAGES_EVMD := $(shell cd postworldd && go list ./... | grep -v '/simulation')
...
TEST_PACKAGES := ./...
TEST_TARGETS := test-unit test-postworldd test-unit-cover test-race
...
test-postworldd: ARGS=-timeout=15m
test-postworldd:
	@cd postworldd && go test -race -tags=test -mod=readonly $(ARGS) $(EXTRA_ARGS) $(PACKAGES_EVMD)
...
test-unit-cover: run-tests
	...
	@echo "🔍 Running postworldd coverage..."
	@cd postworldd && go test -race -tags=test $(COMMON_COVER_ARGS) -coverpkg=$(COVERPKG_ALL) -coverprofile=coverage_postworldd.txt ./...
	@echo "🔀 Merging postworldd coverage into root coverage..."
	@tail -n +2 postworldd/coverage_postworldd.txt >> coverage.txt && rm postworldd/coverage_postworldd.txt
...
test-all:
	...
	@echo "🔍 Running postworldd module tests..."
	@cd postworldd && go test -race -tags=test -mod=readonly -timeout=15m $(PACKAGES_EVMD)
...
localnet-build-env:
	$(MAKE) -C contrib/images postworldd-env

localnet-build-nodes:
	$(DOCKER) run --rm -v $(CURDIR)/.testnets:/data cosmos/postworldd \
...
test-system: build-v04 build
	...
	cp $(BUILDDIR)/postworldd ./tests/systemtests/binaries/
...
build-v04:
	...
	git checkout v0.4.1
	make build
	cp $(BUILDDIR)/postworldd ./tests/systemtests/binaries/v0.4
	git checkout -
