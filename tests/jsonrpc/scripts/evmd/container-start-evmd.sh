...
# Container-friendly postworldd initialization and startup script
# This runs inside the postworldd container, so no Docker commands

set -e

echo "🔧 Starting postworldd container initialization..."

# Set up variables (same as start-postworldd.sh)
KEYRING="test"
...
# Standard test keys (same as start-postworldd.sh)
VAL_KEY="mykey"
...
echo "$VAL_MNEMONIC" | postworldd init localtestnet -o --chain-id "$CHAIN_ID" --recover --home "$CHAINDIR"

# Set client config
postworldd config set client chain-id "$CHAIN_ID" --home "$CHAINDIR"
...
echo "$USER1_MNEMONIC" | postworldd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER2_MNEMONIC" | postworldd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER3_MNEMONIC" | postworldd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
echo "$USER4_MNEMONIC" | postworldd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
