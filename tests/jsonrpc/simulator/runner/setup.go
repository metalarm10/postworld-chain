...
import (
	...
	"github.com/metalarm10/postworld-chain/tests/jsonrpc/simulator/contracts"
	"github.com/metalarm10/postworld-chain/tests/jsonrpc/simulator/types"
	"github.com/metalarm10/postworld-chain/tests/jsonrpc/simulator/utils"
)
...
func Setup() (*types.RPCContext, error) {
	...
	log.Printf("Created filter for ERC20 transfers: postworldd=%s, geth=%s\n", rCtx.Evmd.FilterID, rCtx.Geth.FilterID)
...
// deployContracts deploys the ERC20 contract to both postworldd and geth
...
// mintTokensOnBothNetworks distributes ERC20 tokens to specified accounts on both postworldd and geth
func mintTokensOnBothNetworks(rCtx *types.RPCContext) error {
	...
	// Distribute on postworldd (from dev0 who deployed the contract)
	fmt.Printf("Distributing tokens on postworldd (contract: %s)...\n", rCtx.Evmd.ERC20Addr.Hex())
	postworlddReceipts, err := distributeTokensOnNetwork(rCtx, distributionTargets, config.Dev0PrivateKey, false)
	if err != nil {
		return fmt.Errorf("failed to distribute tokens on postworldd: %w", err)
	}
	...
	postworlddSuccess := 0
	gethSuccess := 0
	for _, receipt := range postworlddReceipts {
		if receipt.Status == 1 {
			postworlddSuccess++
		}
	}
	...
	fmt.Printf("\nDistribution summary: postworldd (%d/%d), geth (%d/%d)\n",
		postworlddSuccess, len(postworlddReceipts), gethSuccess, len(gethReceipts))

	if postworlddSuccess != len(distributionTargets) || gethSuccess != len(distributionTargets) {
...
func newFilter(rCtx *types.RPCContext) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create postworldd filter: %w", err)
	}
