...
import (
	...
	"github.com/metalarm10/postworld-chain/tests/jsonrpc/simulator/config"
)
...
// ComparisonResult holds the result of comparing API response structures between postworldd and geth
type ComparisonResult struct {
	Method         string            `json:"method"`
	EvmdType       string            `json:"postworldd_type"`
	GethType       string            `json:"geth_type"`
	EvmdStructure  map[string]string `json:"postworldd_structure,omitempty"` // Field names -> types
	...
	EvmdError      string            `json:"postworldd_error,omitempty"`
...
func NewRPCContext(conf *config.Config) (*RPCContext, error) {
	// Connect to the primary Ethereum client (postworldd)
...
func (rCtx *RPCContext) CompareRPCCall(method string, params ...interface{}) *ComparisonResult {
	...
	// Call postworldd
	var postworlddResponse interface{}
	postworlddErr := rCtx.Evmd.RPCClient().CallContext(context.Background(), &postworlddResponse, method, params...)
	if postworlddErr != nil {
		result.EvmdError = postworlddErr.Error()
	}
	...
	// Compare errors
	result.ErrorsMatch = (postworlddErr == nil && gethErr == nil) ||
		(postworlddErr != nil && gethErr != nil)

	// Compare structure and types if both succeeded
	if postworlddErr == nil && gethErr == nil {
		result.EvmdType = rCtx.getTypeDescription(postworlddResponse)
		result.GethType = rCtx.getTypeDescription(gethResponse)
		result.TypeMatch = result.EvmdType == result.GethType

		result.EvmdStructure = rCtx.analyzeStructure(postworlddResponse)
...
func (rCtx *RPCContext) CompareRPCCallWithProvider(method string, paramProvider ParameterProvider) *ComparisonResult {
	...
	postworlddParams := paramProvider(false) // false = postworldd
	gethParams := paramProvider(true)  // true = geth

	// Call postworldd
	var postworlddResponse interface{}
	postworlddErr := rCtx.Evmd.RPCClient().CallContext(context.Background(), &postworlddResponse, method, postworlddParams...)
	if postworlddErr != nil {
		result.EvmdError = postworlddErr.Error()
	}
	...
	if postworlddErr == nil && gethErr == nil {
		...
		if postworlddErr != nil && gethErr == nil {
			result.Differences = []string{"postworldd request failed, geth succeeded - cannot compare structures"}
		} else if postworlddErr == nil && gethErr != nil {
			result.Differences = []string{"geth request failed, postworldd succeeded - cannot compare structures"}
		} else {
...
func (rCtx *RPCContext) findStructuralDifferences(postworlddType, gethType string, postworlddStruct, gethStruct map[string]string) []string {
	var differences []string

	// Type comparison
	if postworlddType != gethType {
		differences = append(differences, fmt.Sprintf("Root type mismatch: postworldd=%s, geth=%s", postworlddType, gethType))
	}

	// Find missing fields in postworldd
	for field, fieldType := range gethStruct {
		if _, exists := postworlddStruct[field]; !exists {
			differences = append(differences, fmt.Sprintf("Missing field in postworldd: %s (%s)", field, fieldType))
		}
	}

	// Find extra fields in postworldd
	for field, fieldType := range postworlddStruct {
		if _, exists := gethStruct[field]; !exists {
			differences = append(differences, fmt.Sprintf("Extra field in postworldd: %s (%s)", field, fieldType))
		}
	}

	// Find type mismatches
	for field, postworlddFieldType := range postworlddStruct {
		if gethFieldType, exists := gethStruct[field]; exists && postworlddFieldType != gethFieldType {
			differences = append(differences, fmt.Sprintf("Type mismatch for %s: postworldd=%s, geth=%s", field, postworlddFieldType, gethFieldType))
		}
	}
...
// ParameterProvider is a function that provides parameters for postworldd and geth separately
