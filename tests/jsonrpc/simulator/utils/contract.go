...
func VerifyTokenBalances(rCtx *types.RPCContext) error {
	...
	for _, privateKeyHex := range accounts {
		...
		// Get balance on postworldd
		postworlddBalance, err := getTokenBalance(rCtx, addr, false)
		if err != nil {
			return fmt.Errorf("failed to get postworldd balance for %s: %w", addr.Hex(), err)
		}
		...
		// Compare balances
		if postworlddBalance.Cmp(gethBalance) != 0 {
			return fmt.Errorf("balance mismatch for %s: postworldd=%s, geth=%s",
				addr.Hex(), postworlddBalance.String(), gethBalance.String())
		}

		readableBalance := new(big.Int).Div(postworlddBalance, big.NewInt(1e18))
