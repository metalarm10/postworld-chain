...
func NewERC20FilterLogs(rCtx *types.RPCContext, isGeth bool) (ethereum.FilterQuery, string, error) {
	...
	// Create filter on postworldd
	args, err := ToFilterArg(fErc20Transfer)
	if err != nil {
		return fErc20Transfer, "", fmt.Errorf("failed to create filter args: %w", err)
	}
	var postworlddFilterID string
	if err = ethCli.RPCClient().CallContext(rCtx, &postworlddFilterID, "eth_newFilter", args); err != nil {
		return fErc20Transfer, "", fmt.Errorf("failed to create filter on postworldd: %w", err)
	}

	return fErc20Transfer, postworlddFilterID, nil
}

// Standard dev account addresses (matching postworldd genesis accounts)
...
func DeployContract(rCtx *types.RPCContext, contractByteCode []byte, isGeth bool) (addr common.Address, txHash string, blockNum *big.Int, err error) {
	...
	fmt.Printf("Deploying ERC20 to postworldd using dev0 (%s)...\n", fromAddress.Hex())

	postworlddTxHash, err := deployContractViaDynamicFeeTx(ethCli.Client, privateKey, contractByteCode)
	if err != nil {
		return common.Address{}, "", nil, err
	} else {
		addr, blockNum, err = waitForContractDeployment(ethCli.Client, postworlddTxHash, 30*time.Second)
		if err != nil {
			return common.Address{}, "", nil, err
		}
	}

	fmt.Printf("✓ postworldd deployment successful: %s\n", addr.Hex())
	return addr, postworlddTxHash, blockNum, nil
}
...
func waitForContractDeployment(client *ethclient.Client, txHashStr string, timeout time.Duration) (common.Address, *big.Int, error) {
	fmt.Printf("Waiting for postworldd deployment (tx: %s)...\n", txHashStr)
