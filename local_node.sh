...
CHAINID="${CHAIN_ID:-postworld_9000-1}"
...
LOGLEVEL="info"
# Set dedicated home directory for the postworldd instance
CHAINDIR="$HOME/.postworldd"
...
Options:
  ...
  --mnemonic-file PATH     Where to write mnemonics YAML (default: \$HOME/.postworldd/mnemonics.yaml)
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -n)
      ...
      ;;
    --no-install)
      echo "Flag --no-install passed -> Skipping installation of the postworldd binary."
      install=false; shift
      ;;
    --remote-debugging)
...
add_genesis_funds() {
  local keyname="$1"
  postworldd genesis add-genesis-account "$keyname" 1000000000000000000000asurvive --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  postworldd config set client chain-id "$CHAINID" --home "$CHAINDIR"
  postworldd config set client keyring-backend "$KEYRING" --home "$CHAINDIR"

  # ---------------- Validator key ----------------
  VAL_KEY="mykey"
  VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"
  echo "$VAL_MNEMONIC" | postworldd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  echo "$VAL_MNEMONIC" | postworldd init $MONIKER -o --chain-id "$CHAINID" --home "$CHAINDIR" --recover
  ...
  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for postworldd.","denom_units":[{"denom":"asurvive","exponent":0,"aliases":["attotest"]},{"denom":"survive","exponent":18,"aliases":[]}],"base":"asurvive","display":"survive","name":"Survive Token","symbol":"SURVIVE","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"asurvive",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  for ((i=0; i<${#dev_mnemonics[@]}; i++)); do
    ...
    echo "$mnemonic" | postworldd keys add "$keyname" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  if [[ "$ADDITIONAL_USERS" -gt 0 ]]; then
    ...
    for ((i=0; i<ADDITIONAL_USERS; i++)); do
      ...
      mnemonic_out="$(postworldd keys add "$keyname" --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR" 2>&1)"
  ...
  # --------- Finalize genesis ---------
  postworldd genesis gentx "$VAL_KEY" 1000000000000000000000asurvive --gas-prices ${BASEFEE}asurvive --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
...
# Start the node
postworldd start "$TRACE" \
	--pruning nothing \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0asurvive \
	--evm.min-tip=0 \
	--home "$CHAINDIR" \
