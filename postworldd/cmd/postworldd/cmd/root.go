...
import (
	...
	cosmosevmcmd "github.com/metalarm10/postworld-chain/client"
...
// NewRootCmd creates a new root command for postworldd. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	...
	tempApp := postworldd.NewExampleApp(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
...
func initRootCmd(rootCmd *cobra.Command, evmApp *postworldd.EVMD) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	var exampleApp *postworldd.EVMD
	...
	if height != -1 {
		exampleApp = postworldd.NewExampleApp(logger, db, traceStore, false, appOpts, config.EVMChainID, config.EvmAppOptions, baseapp.SetChainID(chainID)) // TODO:VLAD - Remove appoptions and evmchainid

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = postworldd.NewExampleApp(logger, db, traceStore, true, appOpts, config.EVMChainID, config.EvmAppOptions, baseapp.SetChainID(chainID)) // TODO:VLAD - Remove // TODO:VLAD - Remove appoptions and evmchainid
	}
